...
# Colonnes du format texte FDSNWS-event (RFC standard) :
...
class IpgpConnector(SeismicConnector):
    code = "IPGP"
    ...
    async def fetch_events(
        ...
        *,
        ...
        response = await self._get(self.settings.ipgp_base_url, params)
        ...
        for line in lines:
            ...
            events.append(
                EarthquakeEvent(
                    id=f"ipgp-{row['event_id']}",
                    externalId=row["event_id"],
                    magnitude=magnitude,
                    magnitudeType=row["mag_type"] or "Ml",
